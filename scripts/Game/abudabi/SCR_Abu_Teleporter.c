...
class SCR_Abu_Teleporter
{
	...
	// Used in: GenerateRandomSpawnPointAttackSide
	float MIN_DISTANCE_ATTACK_DEFEND = 2000.0; // Minimum distance between attack and defend spawns
	float MAX_DISTANCE_ATTACK_DEFEND = 3500.0; // Maximum distance between attack and defend spawns
...
//	}
	...
	static bool SurfaceIsPebbles(vector pos)
	{
		ResourceName material = GetSurfaceMaterial(pos);
		return material.IndexOf("Pebbles") >= 0;
	}
	...
	vector GenerateRandomSpawnPointAttackSide(vector defendPosition)
	{
		...
		while (attempts < MAX_ATTEMPTS_ATTACK_POSITION)
		{
			...
			if (!isExcluded &&
				...
				!SurfaceIsWater(randomPosition) && SurfaceIsFlat(randomPosition) && !SurfaceIsPebbles(randomPosition) &&
			...
			for (float offsetX = -radius; offsetX <= radius; offsetX += SEARCH_STEP)
			{
				for (float offsetZ = -radius; offsetZ <= radius; offsetZ += SEARCH_STEP)
				{
					...
					if (isValid &&
						...
						!SurfaceIsPebbles(candidatePosition) &&
